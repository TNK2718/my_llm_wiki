...
**検索時は text2sql で構造化取得 → 出典付き回答**するパターンの雛形。
...
| text2sql 生成・SQL 検証・自己修復 | `query.py` |
...
- 質問は SLM が text2sql で SELECT 1 文に変換し、構造化結果を取得する。
- read-only 接続・`SELECT` 限定・複文/DDL/PRAGMA 禁止・`LIMIT` 強制・`EXPLAIN` 構文検証。
- 失敗時はエラー文を食わせて **1回だけ自己修復**。それでも駄目なら全文検索のみで回答。
...
- グラフが大きくなり SQL 表現力が要るなら DuckDB / Kuzu への移行余地。
